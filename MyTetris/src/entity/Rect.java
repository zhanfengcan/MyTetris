...
public class Rect {
	...
	public void draw(Graphics g,int pos_x,int pos_y){
		...
		for(int i=0;i<4;i++){
			...
			//g.drawImage(RECT_IMG, pos_x+x[i]*RECTW , pos_y+y[i]*RECTH, pos_x+(x[i]+1)*RECTW , pos_y+(y[i]+1)*RECTH, color*RECTW, 0 , (color+1)*RECTW, RECTH, null);
	...
	public void drawone(Graphics g,int pos_x,int pos_y,int x,int y){
			...
			g.drawImage(RECT_IMG, pos_x+x*RECTW , pos_y+y*RECTH, pos_x+(x+1)*RECTW , pos_y+(y+1)*RECTH, color*RECTW, 0 , (color+1)*RECTW, RECTH, null);
